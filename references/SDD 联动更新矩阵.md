...
本矩阵用于把《SDD 模板总则》中的“文档联动更新规则”进一步具体化。  
它回答的是：
...
### 4.0A 通用阶段起步动作清单（适用于多个阶段起步）
当某阶段刚开始正式推进时，默认至少执行以下动作：
1. 确认前置阶段已确认
2. 先将当前阶段主文档落盘为草案
3. 再继续该阶段内容讨论、收口与门禁判断
4. `sdd-status.md` 的当前阶段与中断点必须同步到“当前阶段草案已落盘”的真实状态

默认原则：
> 阶段起步先落主文档，再展开内容；不得先在对话里推进很多，再回头补当前阶段主文档。
